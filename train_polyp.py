...
#from lib.networks_dualconv import PVT_CASCADE
...
def train(train_loader, model, optimizer, epoch, test_path, model_name = 'DualDHA3'):
    model.train()
    global best
    ...
    if (epoch + 1) % 1 == 0:
        total_dice = 0
        total_images = 0  # 对齐total_dice缩进层级
    
        # 测试数据集列表（对应TestDataset下的子目录）
        for dataset in ['CVC-300', 'CVC-ClinicDB', 'Kvasir', 'CVC-ColonDB', 'ETIS-LaribPolypDB']:
            dataset_dice, n_images = test(model, test_path, dataset)
            total_dice += (n_images * dataset_dice)
            total_images += n_images  # 保持循环体内统一缩进

            # 日志记录（对齐数据集遍历层级）
            logging.info('epoch: {}, dataset: {}, dice: {}'.format(epoch, dataset, dataset_dice))
            print(dataset, ': ', dataset_dice)
            dict_plot[dataset].append(dataset_dice)  # 字典更新与日志同级
        ...
        if meandice > best:
            ...
            best = meandice
            torch.save(model.state_dict(), save_path + '' + model_name + '.pth')  # 路径与目录结构一致
            torch.save(model.state_dict(), save_path + str(epoch) + '' + model_name + '-best.pth')  # 文件名规范
if __name__ == '__main__':
    dict_plot = {'CVC-300':[], 'CVC-ClinicDB':[], 'Kvasir':[], 'CVC-ColonDB':[], 'ETIS-LaribPolypDB':[], 'test':[]}
    name = ['CVC-300', 'CVC-ClinicDB', 'Kvasir', 'CVC-ColonDB', 'ETIS-LaribPolypDB', 'test']
    ##################model_name#############################
    model_name = 'DualHDA3_polyp'
    ###############################################
